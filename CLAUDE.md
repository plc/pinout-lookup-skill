...
- `components.json` -- auto-generated manifest of all components. The skill fetches this single file to search for components instead of listing directories. **Do not edit manually** -- it is regenerated by the PR review workflow's structure job (and can be manually triggered via `manifest.yml` workflow dispatch).
...
- `boards/`, `displays/`, `sensors/`, `motors/`, `audio/` -- component pinout files
...
`components.json` is auto-generated by `scripts/generate-manifest.py` during the PR review workflow's structure job. Manual edits will be overwritten on the next PR. If the manifest needs fixing, fix the script or the source pinout files. A manual `workflow_dispatch` trigger on `manifest.yml` is available as a fallback if the manifest gets out of sync.
...
1. Create a `.md` file in the right directory (`boards/`, `displays/`, `sensors/`, `motors/`, `audio/`, or a new category)
