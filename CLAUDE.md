...
- `components.json` -- auto-generated manifest of all components. The skill fetches this single file to search for components instead of listing directories. **Do not edit manually** -- it is regenerated by the `manifest.yml` GitHub Action on every push to main.
- `scripts/generate-manifest.py` -- generates `components.json` from the pinout files.
...
The skill uses WebFetch for fetching pinout data and the component manifest (works everywhere). `gh` is optional and used for creating issues and opening PRs. If `gh` is missing, the skill gives the user a direct GitHub link instead.

### Do not edit components.json manually

`components.json` is auto-generated by `scripts/generate-manifest.py` and committed by the `manifest.yml` GitHub Action. Manual edits will be overwritten on the next push to main. If the manifest needs fixing, fix the script or the source pinout files.

### New component directories need a workflow path update

The `manifest.yml` workflow only triggers on changes to directories listed in its `paths:` filter (`boards/`, `displays/`, `sensors/`). If you add a new component directory (e.g. `actuators/`), add it to the workflow's paths list too, or the manifest won't regenerate.
...
7. `components.json` is auto-updated by CI -- no manual step needed
